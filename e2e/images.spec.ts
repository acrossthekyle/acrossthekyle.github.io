...
test('images page loads snapshot', async ({ page }) => {
  await page.goto('/images/photos/pyramids-cairo/01');
  ...
  await expect(heading).toBeVisible();
  await expect(heading).toContainText('Pyramids, Cairo');
  ...
  await expect(camera).toBeVisible();
  await expect(camera).toContainText('iPhone 13 Pro');

  const iso = await items.filter({ hasText: 'iso' });

  await expect(iso).toBeVisible();
  await expect(iso).toContainText('50');

  const focal = await items.filter({ hasText: 'focal length' });

  await expect(focal).toBeVisible();
  await expect(focal).toContainText('42 mm');
  ...
  await expect(fNumber).toBeVisible();
  await expect(fNumber).toContainText('f/1.50');

  const aperture = await items.filter({ hasText: 'Aperture' });

  await expect(aperture).toBeVisible();
  await expect(aperture).toContainText('1/1699 s');
});
...
test('images page snapshot has next/previous', async ({ page }) => {
  await page.goto('/images/photos/pyramids-cairo/01');
  ...
  await expect(page).toHaveURL('/images/photos/highlands-scotland/02');
  ...
  await expect(page).toHaveURL('/images/photos/pyramids-cairo/01');

  await page.goto('/images/photos/aqua-tower-chicago/05');
